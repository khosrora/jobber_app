...
import { IAuth } from '@gateway/helper/auth.interface';
...
class AuthService {
  ...
  async resetPassword(token: string, newPassword: string, confirmPassword: string): Promise<AxiosResponse> {
    const response: AxiosResponse = await this.axiosService.axios.post(`/reset-password/${token}`, { newPassword, confirmPassword });
    return response;
  }
