...
import { NotAuthorizedError } from '@users/helper/error-handler';
...
export function verifyGatewayRequest(req: Request, _res: Response, next: NextFunction): void {
  ...
  try {
    const payload: { id: string; iat: number } = JWT.verify(token, process.env.GATEWAY_JWT_TOKEN!) as { id: string; iat: number };
