...
const consumeReviewFanoutMessages = async (channel: Channel): Promise<void> => {
  try {
    ...
    const jobberQueue: Replies.AssertQueue = await channel.assertQueue(queueName, { durable: true, autoDelete: false });
    // because we use fanout . we dont need routingkey
...
const consumeSeedGigsDirectMessages = async (channel: Channel): Promise<void> => {
  try {
    if (!channel) {
      channel = (await createConnection()) as Channel;
    }
    ...
    channel.consume(jobberQueue.queue, async (msg: ConsumeMessage | null) => {
      ...
      if (type === 'getSellers') {
        ...
        await publishDirectMessage(
          ...
          'jobber-seed-gig',
          'recive-sellers',
          JSON.stringify({ type: 'reviceSellers', sellers, count }),
          'Message sent to gig service'
        );
      }
      channel.ack(msg!);
    ...
    log.log('error', 'UsersService UserConsumer consumeSeedGigsDirectMessages() method error : ', error);
  }
};

export { consumeBuyerDirectMessage, consumeReviewFanoutMessages, consumeSellerDirectMessage, consumeSeedGigsDirectMessages };
