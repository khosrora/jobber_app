...
import { IRatingTypes, IReviewMessageDetails } from '@gig/helper/review.interface';
...
import { gigsSearchsBySellerId } from '@gig/services/search.service';
import { faker } from '@faker-js/faker';
import { sample } from 'lodash';
import { ISellerDocument } from '@gig/helper/seller.interface';
...
const getSellerGigs = async (sellerId: string): Promise<ISellerGig[]> => {
  ...
  const gigs = await gigsSearchsBySellerId(sellerId, true);
...
const getSellerPausedGigs = async (sellerId: string): Promise<ISellerGig[]> => {
  const resultHits: ISellerGig[] = [];
  const gigs = await gigsSearchsBySellerId(sellerId, false);
  for (const item of gigs.hits) {
    resultHits.push(item._source as ISellerGig);
  }
  return resultHits;
...
const updateActiveProp = async (gigId: string, gigActive: boolean): Promise<ISellerGig> => {
  const document: ISellerGig = (await GigModel.findOneAndUpdate(
    {
      _id: gigId
    },
...
const updateGigReview = async (data: IReviewMessageDetails): Promise<void> => {
  ...
  if (gig) {
    ...
    await updateIndexData('gigs', `${gig._id}`, data);
  }
};
...
export { getGigById, getSellerGigs, getSellerPausedGigs, createGig, deleteGig, updateGig, updateActiveProp, updateGigReview, seedData };
