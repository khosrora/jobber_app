...
import { IHitsTotal, IPaginateProps, IQueryList, ISearchResult } from '@gig/helper/search.interface';

const gigsSearchsBySellerId = async (searchQuery: string, active: boolean): Promise<ISearchResult> => {
...
const gigsSearch = async (
  ...
  const queryList: IQueryList[] = [
    {
      query_string: {
        fields: ['username', 'title', 'description', 'basicDescription', 'basicTitle', 'categories', 'categories', 'subCategories', 'tags'],
        query: `*${searchQuery}*`
      }
    },
    {
...
export { getMoreGigsLikeThis, gigsSearchByCategory, gigsSearch, gigsSearchsBySellerId, getTopRatedGigsByCategory };
