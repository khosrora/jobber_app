import { IConversationDocument, IMessageDocument } from '@chat/helper/chat.interface';
...
const conversation = async (req: Request, res: Response): Promise<void> => {
  ...
  res.status(StatusCodes.OK).json({ message: 'Chat Messages', conversations });
};
...
const userMessages = async (req: Request, res: Response): Promise<void> => {
  ...
  res.status(StatusCodes.OK).json({ message: 'Chat list', messages });
};
