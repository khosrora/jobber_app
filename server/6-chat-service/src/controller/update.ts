...
import { markManyMesageAsRead, markMessageAsRead, updateOffer } from '@chat/services/message.service';
...
const markMultipleMessages = async (req: Request, res: Response): Promise<void> => {
  ...
  await markManyMesageAsRead(receiverUsername, senderUsername, messageId);
  res.status(StatusCodes.OK).json({ message: 'Message updated' });
};
